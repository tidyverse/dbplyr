---
title: "Developing a dbplyr backend"
...
All dbplyr pipelines are built on top of the the `tbl_lazy` data structure, which is the only data structure that the user will ever see. It has two fields:
...
Each verb verb takes a `tbl_lazy` as input and returns a modified `tbl_lazy`, modifying the `lazy_query` with the new requirements. At the simplest level, this involves wrapping the existing lazy query within a new lazy query, such as `lazy_query_select()` or `lazy_query_multi_join()`. These represent what later will become a subquery so that (e.g.) `filter()`, `mutate()`, `select()`, `arrange()`, and `summarise()` all use `lazy_select_query()`, where as `left_join()` and `inner_join()` uses `lazy_query_multi_join()`. 
...
2. They are responsible for figuring out if we can re-use the existing query or we need to wrap in a subquery. Typically this work is done by a function called `select_can_inline()` which will use what we know about SQL and the verb to determine whether or not we have to introduce a subquery.
...
First, `sql_build()` recurses over the lazy query building up query objects  like `select_query()` and `join_query()` that represent the different subtypes of `SELECT` queries. Compared to the previous "lazy" query representation these are much closer to SQL than R. They can be much simpler because we no longer need to reason about subqueries, but we still need some structure to faciliate backend-specific tranlations.

This is the next step: `sql_render()` is called on the result of `sql_build()`, dispatches to generics like `sql_query_select()`, `sql_query_join()`, `sql_query_semi_join()`, `sql_query_set_op()`. This generics have methods for each backend that are responsible for creating the needed SQL.
