...
*   If `copy_to()` fails, it's likely you need a method for 
    `sql_write_table()`, `sql_create_index()` or `sql_analyze()`.
  
    If it fails during creation of the tbl, you may need a method for
    `sql_query_fields()`.
