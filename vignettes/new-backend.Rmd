...
If you need to generate your own SQL, we recommend using `glue_sql2()`. It uses glue syntax with type markers for safe SQL generation:
...
glue_sql2(
  con,
  "CREATE INDEX {.sql index_name} ON {.tbl table} ({.id columns*})"
)
...
glue_sql2(con, "SELECT {.id cols*} FROM {.tbl table}")
```

Type markers include `.tbl` (tables), `.id` (identifiers), `.val` (values), `.name` (names), `.from` (subqueries/tables), and `.kw` (keywords). Use `*` after `.id` or `.val` to collapse vectors into comma-separated lists.
...
For more complex translations, you can write custom functions that return SQL expressions using `sql_expr()`. This is a tidy-evaluation function that allows you to use `!!` operator to in inject a single value into SQL expressions or `!!!` to splice a list of values.
