...
# Some arrange the arguments differently:
...
```{r}
translate_sql(mean(G), window = TRUE)
translate_sql(rank(G), window = TRUE)
translate_sql(ntile(G, 2), window = TRUE)
translate_sql(lag(G), window = TRUE)
```
...
```{r}
translate_sql(cummean(G), vars_order = "year", window = TRUE)
translate_sql(rank(), vars_group = "ID", window = TRUE)
```
...
`translate_sql()` takes an optional `con` parameter. If not supplied, this causes dplyr to generate (approximately) SQL-92 compliant SQL. If supplied, dplyr uses `sql_translate_env()` to look up a custom environment which makes it possible for different databases to generate slightly different SQL.
