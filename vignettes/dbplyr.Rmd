...
You'll also need to install a DBI backend package. A backend package takes care of communicating between R and the database in a standardised way that dplyr builds on top of. Here are four commonly used backends:
...
In this vignette, we're going to use the RSQLite backend which is automatically installed when you install dbplyr. SQLite is a great way to get started with databases because it's completely embedded inside an R package. Unlike most other systems, you don't need to setup a separate database server. SQLite is create for demos, but is surprisingly powerful, and with a little knowledge can easily allow to work with many gigabytes of data.

## Connecting to the database

To work with a database in dplyr, you must first make a connection to that database using `DBI::dbConnect()` (DBI is also automatically installed when you install dbplyr). We're not going to go into the details of the DBI package here, but it's the foundation upon which dplyr is built. You'll need to learn more about if you need to do things to the database that are beyond the scope of dplyr.
...
(If you're not using RStudio, you'll need some other way to securely store your password. You should never record it in your analysis scripts or type it into the console.)
...
As you can see, the `copy_to()` operation has an additional argument that allows you to supply indexes for the table. Here we set up indexes that will allow us to quickly process the data by day, by carrier and by plane.
...
To interact with a database you usually use SQL, Structured Query Language. SQL is over 40 years old, and is used by pretty much every database in existence. The goal of dbplyr is to automatically generate SQL for you. But SQL is a very large language so dbplyr doesn't do everything: instead it focusses on `SELECT` statements, the SQL you write most often as an analyst.

For basic dplyr usage, you don't need to know anything about SQL, but in the long-run I highly recomend you at least learn the basics. If you have some familiarity with SQL and you'd like to learn more, I found [how indexes work in SQLite](http://www.sqlite.org/queryplanner.html) and [10 easy steps to a complete understanding of SQL](http://blog.jooq.org/2016/03/17/10-easy-steps-to-a-complete-understanding-of-sql) to be particularly helpful.

While dplyr is generating SQL behind the scenes, you generally don't need to know anything about it, and can continue to use the dplyr verbs you're already familiar with:
...
The most important difference is that your R code is translated into SQL and executed in the database, not in R. When working with databases, dplyr tries to be as lazy as possible:
...
You can learn more about the SQL translation in `vignette("sql-translation")`.

This probably isn't exactly what you'd write by hand, but it does the job.
...
You can also ask the database how it plans to execute the query with `explain()`. The output is often fairly esoteric, but learning a bit about it can be very useful because it helps you understand if the database is using it's indexes efficiently.
