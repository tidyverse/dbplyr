...
You can see the query dplyr will generate to pull down results by using the `show_query()` function from dbplyr:

```{r}
c4 %>% dbplyr::show_query()
```
...
flights_db %>% head(6) %>% dbplyr::show_query()
```
...
bestworst %>% dbplyr::show_query()

# Rank each flight within a daily
ranked <- daily %>% 
  select(arr_delay) %>% 
  mutate(rank = rank(desc(arr_delay)))
ranked %>% dbplyr::show_query()
```
