...
`dbplyr::translate_sql()` powers translation of individual function calls, and I'll use it extensively in this vignette to show what's happening. Ordinarily, you shouldn't need to use it in your code as dbplyr takes care of the translation automatically.
...
dbplyr translates `%%` to the SQL equivalents but note that it's not precisely the same: most databases use truncated division, where the modulo operator takes the sign of the dividend; on the other hand, R uses the mathematically preferred floored division with the modulo sign taking the sign of the divisor.
...
Because SQL functions are, in general, case insensitive, I recommend using upper case when you're using SQL functions in R code. That makes it easier to spot that you're doing something unusual:
...
* The __frame clause__ defines which rows, or __frame__, are passed 
  to the window function, describing which rows (relative to the current row)
  should be included. The frame clause provides two offsets that determine
  ...
  confusing, and is summarised visually below.
    ...
    Of the many possible specifications, there are only three that are commonly
    ...
    * Rolling: `BETWEEN 2 PRECEEDING AND 2 FOLLOWING`
    ...
    dplyr generates the frame clause based on whether you are using a recycled
...
* For ranking functions, the ordering variable is the first argument: `rank(x)`,
  `ntile(y, 2)`. If omitted or `NULL`, dplyr will use the default ordering associated 
