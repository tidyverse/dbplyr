...
```{r}
lf |> transmute(z = FOOFIFY(x, y))
```

If you are writing code like this inside a package, `R CMD CHECK` will complain about `FOOFIFY` not being defined. You can silence this note using `globalVariables("FOOFIFY")` but this gets tedious if you use many SQL functions. So instead you can use the `.sql` prefix:

```{r}
lf |> transmute(z = .sql$FOOFIFY(x, y))
```

`.sql$` is stripped off automatically in the translation, but it means that you can do a single `globalVariables(".sql")` call, instead of one for each function.
...
To see how individual window functions are translated to SQL, we can use `mutate()`:
