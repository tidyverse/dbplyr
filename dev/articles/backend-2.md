...
This transition guide is aimed at backend authors. dbplyr 2.0.0 is an
...
give backend authors plenty of time to make changes.

The current timeline is something like this:

- dbplyr 2.0.0 adds a new interface for database backends. The old
...
- dbplyr 2.2.0 (released \>= 18 months dbplyr 2.0.0) deprecates the old
  interface, so that users are encouraged to upgrade backends.

- dbplyr 2.3.0 (to be released \>= 12 months after dbplyr 2.2.0) removes
  the old interface so user must upgrade backends.

- A future version of dplyr will deprecate then remove the database
  generics.

## Unused generics

A number of generics are no longer used so you can delete the
...
- [`db_write_table()`](https://dplyr.tidyverse.org/reference/backend_dbplyr.html)
  ...
  instead of nine individual generics:
...
- [`sql_escape_ident()`](https://dplyr.tidyverse.org/reference/backend_dbplyr.html)
  ...
  are no longer used in favour of calling `dbQuoteIdentifier()` and
  `dbQuoteString()` directly.

- [`db_query_rows()`](https://dplyr.tidyverse.org/reference/backend_dbplyr.html)
  was never actually used.

Making these changes are important because they ensure your backend
works consistently whether you use it through DBI or dplyr.

## 2nd edition

dbplyr 2.0.0 draws inspiration from the idea of an
[edition](https://testthat.r-lib.org/articles/third-edition.html) so
that to tell dbplyr to use the new generics, you need to do two things:

- Depend on dbplyr 2.0.0 in your `DESCRIPTION`, e.g.
  `Imports: dbplyr (>= 2.0.0)`. This ensures that when someone installs
  your package they get the latest version of dbplyr.
...
(and dbplyr takes care of executing it):
...
- [`dplyr::db_explain()`](https://dplyr.tidyverse.org/reference/backend_dbplyr.html)
  -\>
  [`dbplyr::sql_query_explain()`](https://dbplyr.tidyverse.org/dev/reference/db-sql.md)
...
If you have methods for any of those generics, you’ll need to extract
the SQL generation code into a new `sql_` method.
...
- [`dplyr::sql_translate_env()`](https://dplyr.tidyverse.org/reference/backend_dbplyr.html)
  -\>
  [`dbplyr::sql_translation()`](https://dbplyr.tidyverse.org/dev/reference/db-sql.md)
...
You may also want to consider methods for the new generics in dbplyr
2.0.0:

- Provide a method for `db_temporary_table()` if your backend requires
  that temporary tables have special names.

- Provide a method for
  [`sql_expr_matches()`](https://dbplyr.tidyverse.org/dev/reference/db-sql.md)
  if your database has special syntax for matching two values (see
  <https://modern-sql.com/feature/is-distinct-from>).

- Provide a method for
  [`sql_join_suffix()`](https://dbplyr.tidyverse.org/dev/reference/db-misc.md)
  if your backend can’t use the usual `.x` and `.y` suffixes in joins.
