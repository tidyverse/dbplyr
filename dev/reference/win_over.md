...
specification. `win_absent()`, `win_rank()`, `win_aggregate()`, and
`win_cumulative()` provide helpers for constructing common types of
window functions. `win_current_group()` and `win_current_order()` allow
you to access the grouping and order context set up by
