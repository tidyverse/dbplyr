...
#' @returns When `in_place = FALSE`, a new `tbl_lazy`.
#'   When `in_place = TRUE` and `returning = NULL`, the modified table, invisibly.
#'   When `in_place = TRUE` and `returning` is set, the values of the selected
#'   columns. This is primarily useful if you need to retrieve the values
#'   of autogenerated primary keys.
#' @rdname rows-db
#' @examples
#' library(dplyr)
#'
...
#' ponies
#'
#' # But you can also choose to modify the table by setting
#' rows_insert(ponies, applejack, conflict = "ignore", in_place = TRUE)
#' # In this case `rows_insert()` returns nothing and the underlying
#' # data is modified
#' ponies
#'
#' fluttershy <- copy_inline(con, data.frame(
#'   name = "Fluttershy",
#'   cutie_mark = "three butterflies"
#' ))
#' # If the table has an auto incrementing primary key, you can use
#' # the returning argument to retrieve its value
#' new_rows <- rows_insert(
#'   ponies,
#'   fluttershy,
#'   conflict = "ignore",
#'   in_place = TRUE,
