test_that("custom scalar translated correctly", {
  ...
  expect_equal(
    test_translate_sql(as.numeric(x)),
    sql("CAST(`x` AS DECIMAL(12, 9))")
  )
  expect_equal(
    test_translate_sql(as.numeric(x, 8)),
    sql("CAST(`x` AS DECIMAL(12, 8))")
  )
  expect_equal(test_translate_sql(as.double(x)), sql("CAST(`x` AS FLOAT)"))
  expect_equal(
    test_translate_sql(as.character(x)),
    sql("CAST(`x` AS VARCHAR(255))")
  )
  expect_equal(
    test_translate_sql(as.character(x, 12)),
    sql("CAST(`x` AS VARCHAR(12))")
  )
  ...
  expect_equal(
    ...
    sql("SUBSTR(`x`, 1.0, 2.0)")
  )
  expect_equal(
    test_translate_sql(startsWith(x, "A")),
    sql("CAST(CASE WHEN INSTR(`x`, 'A') = 1 THEN 1 ELSE 0 END AS INTEGER)")
  )
  expect_equal(test_translate_sql(paste0(x, y)), sql("`x` || `y`"))
  expect_equal(test_translate_sql(paste(x, y)), sql("`x` || ' ' || `y`"))
  expect_equal(
    test_translate_sql(as.Date("2020-01-01")),
    sql("DATE '2020-01-01'")
  )
  expect_equal(
    test_translate_sql(as.Date(sql("x + 1"))),
    sql("CAST(x + 1 AS DATE)")
  )
  expect_equal(test_translate_sql(as.Date(x)), sql("CAST(`x` AS DATE)"))
  expect_equal(
    test_translate_sql(week(as.Date("2020-01-01"))),
    sql("WEEKNUMBER_OF_YEAR(DATE '2020-01-01', 'iso')")
  )
  expect_equal(
    test_translate_sql(quarter(as.Date("2020-01-01"))),
    sql("to_char(DATE '2020-01-01', 'q')")
  )
})
...
test_that("custom aggregators translated correctly", {
  ...
  expect_equal(
    test_translate_sql(cumsum(x, order_by = c(A, B))),
    sql("SUM(`x`) OVER (ORDER BY `A`, `B` ROWS UNBOUNDED PRECEDING)")
  )
})
