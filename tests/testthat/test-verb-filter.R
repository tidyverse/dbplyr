test_that("filter captures local variables", {
  mf <- local_memdb_frame("df", x = 1:5, y = 5:1)
...
test_that("can group transiently using `.by`", {
  df <- local_memdb_frame("df", g = c(1, 1, 2, 1, 2), x = c(5, 10, 1, 2, 3))
...
test_that("filter calls windowed versions of sql functions", {
  df1 <- local_memdb_frame("df", x = 1:10, g = rep(c(1, 2), each = 5))
...
test_that("cumulative aggregates generate window function", {
  df1 <- local_memdb_frame("df", x = c(1:3, 2:4), g = rep(c(1, 2), each = 3))
  out <- df1 |>
    group_by(g) |>
    window_order(x) |>
    filter(cumsum(x) > 3)
...
test_that("filter() after summarise() uses `HAVING`", {
  ...
  mf <- local_memdb_frame("df", g = c(1, 1, 1, 2, 2), h = 1, x = 1:5) |>
    group_by(g, h) |>
    summarise(x_mean = mean(x, na.rm = TRUE), .groups = "drop_last")
