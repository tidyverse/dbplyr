test_that("group_by with .add = TRUE adds groups", {
  mf <- local_memdb_frame("df", x = 1:3, y = 1:3)
  gf1 <- mf |> group_by(x, y)
...
test_that("collect, collapse and compute preserve grouping", {
  g <- local_memdb_frame("df", x = 1:3, y = 1:3) |> group_by(x, y)
...
test_that("joins preserve grouping", {
  g <- local_memdb_frame("df", x = 1:3, y = 1:3) |> group_by(x)
...
test_that("group_by can perform mutate", {
  mf <- local_memdb_frame("df", x = 3:1, y = 1:3)
  out <- mf |>
    group_by(z = x + y) |>
    summarise(n = n()) |>
    collect()
