...
test_that("works with groups", {
  df <- local_memdb_frame(g = 1, x = 1, w = 1) |> dplyr::group_by(g)
...
test_that("must evaluate to integer", {
  df <- local_memdb_frame(x = 1, w = 1 / 2)
...
test_that("works with 0 weights", {
  df <- local_memdb_frame(x = 1:2, w = c(0, 1))
