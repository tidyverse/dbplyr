...
test_that("distinct adds DISTINCT suffix", {
  out <- local_memdb_frame("df", x = c(1, 1)) |> distinct()
...
test_that("distinct can compute variables", {
  out <- local_memdb_frame("df", x = c(2, 1), y = c(1, 2)) |>
    distinct(z = x + y)
