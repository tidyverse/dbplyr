test_that("custom scalar translated correctly", {
  ...
  expect_equal(
    test_translate_sql(round(x, digits = 1.1)),
    sql("ROUND((`x`) :: FLOAT, 1)")
  )
  expect_equal(
    test_translate_sql(grepl("exp", x)),
    sql("REGEXP_INSTR(`x`, 'exp', 1, 1, 0, 'c') != 0")
  )
  expect_equal(
    test_translate_sql(grepl("exp", x, ignore.case = TRUE)),
    sql("REGEXP_INSTR(`x`, 'exp', 1, 1, 0, 'i') != 0")
  )
})
...
test_that("pasting translated correctly", {
  ...
  expect_equal(
    test_translate_sql(paste(x, y)),
    sql("ARRAY_TO_STRING(ARRAY_CONSTRUCT_COMPACT(`x`, `y`), ' ')")
  )
  expect_equal(
    test_translate_sql(paste0(x, y)),
    sql("ARRAY_TO_STRING(ARRAY_CONSTRUCT_COMPACT(`x`, `y`), '')")
  )
  expect_equal(test_translate_sql(str_c(x, y)), sql("CONCAT_WS('', `x`, `y`)"))
  expect_equal(
    test_translate_sql(str_c(x, y, sep = "|")),
    sql("CONCAT_WS('|', `x`, `y`)")
  )
  ...
  expect_equal(
    test_translate_sql(str_flatten(x), window = TRUE),
    sql("LISTAGG(`x`, '') OVER ()")
  )
  expect_equal(
    test_translate_sql(str_flatten(x, collapse = "|"), window = TRUE),
    sql("LISTAGG(`x`, '|') OVER ()")
  )
  expect_equal(
    test_translate_sql(str_flatten(x, collapse = "|"), window = FALSE),
    sql("LISTAGG(`x`, '|')")
  )
})
...
test_that("custom stringr functions translated correctly", {
  ...
  expect_equal(
    test_translate_sql(str_detect(x, y)),
    sql("REGEXP_INSTR(`x`, `y`) != 0")
  )
  expect_equal(
    test_translate_sql(str_detect(x, y, negate = TRUE)),
    sql("REGEXP_INSTR(`x`, `y`) = 0")
  )
  expect_equal(
    test_translate_sql(str_replace(x, y, z)),
    sql("REGEXP_REPLACE(`x`, `y`, `z`, 1.0, 1.0)")
  )
  expect_equal(
    test_translate_sql(str_replace(x, "\\d", z)),
    sql("REGEXP_REPLACE(`x`, '\\\\d', `z`, 1.0, 1.0)")
  )
  expect_equal(
  ...
  expect_equal(
    test_translate_sql(str_squish(x)),
    sql("REGEXP_REPLACE(TRIM(`x`), '\\\\s+', ' ')")
  )
  expect_equal(
    test_translate_sql(str_remove(x, y)),
    sql("REGEXP_REPLACE(`x`, `y`, '', 1.0, 1.0)")
  )
  expect_equal(
    test_translate_sql(str_remove_all(x, y)),
    sql("REGEXP_REPLACE(`x`, `y`)")
  )
  expect_equal(test_translate_sql(str_trim(x)), sql("TRIM(`x`)"))
  expect_equal(
    test_translate_sql(str_starts(x, y)),
    sql("REGEXP_INSTR(`x`, `y`) = 1")
  )
  expect_equal(
    test_translate_sql(str_starts(x, y, negate = TRUE)),
    sql("REGEXP_INSTR(`x`, `y`) != 1")
  )
  expect_equal(
    test_translate_sql(str_ends(x, y)),
...
test_that("aggregates are translated correctly", {
  ...
  expect_equal(
    test_translate_sql(cor(x, y), window = FALSE),
    sql("CORR(`x`, `y`)")
  )
  expect_equal(
    test_translate_sql(cor(x, y), window = TRUE),
    sql("CORR(`x`, `y`) OVER ()")
  )

  expect_equal(
    test_translate_sql(cov(x, y), window = FALSE),
    sql("COVAR_SAMP(`x`, `y`)")
  )
  expect_equal(
    test_translate_sql(cov(x, y), window = TRUE),
    sql("COVAR_SAMP(`x`, `y`) OVER ()")
  )
  ...
  expect_equal(
    test_translate_sql(all(x, na.rm = TRUE), window = TRUE),
    sql("BOOLAND_AGG(`x`) OVER ()")
  )
  ...
  expect_equal(
    test_translate_sql(any(x, na.rm = TRUE), window = TRUE),
    sql("BOOLOR_AGG(`x`) OVER ()")
  )
  ...
  expect_equal(
    test_translate_sql(sd(x, na.rm = TRUE), window = TRUE),
    sql("STDDEV(`x`) OVER ()")
  )
})
...
test_that("custom lubridate functions translated correctly", {
  ...
  expect_equal(
    test_translate_sql(month(x, label = TRUE)),
    sql("MONTHNAME(`x`)")
  )
  expect_equal(
  ...
  expect_equal(
    ...
    sql("(EXTRACT('year', `x`) || '.' || EXTRACT('quarter', `x`))")
  )
  expect_error(
  ...
  expect_equal(
    test_translate_sql(floor_date(x, "month")),
    sql("DATE_TRUNC('month', `x`)")
  )
  expect_equal(
    test_translate_sql(floor_date(x, "week")),
...
test_that("pmin() and pmax() respect na.rm", {
  ...
  expect_equal(
    test_translate_sql(pmin(x, y, z, na.rm = FALSE)),
    sql("LEAST(`x`, `y`, `z`)")
  )
  expect_equal(
    test_translate_sql(pmax(x, y, z, na.rm = FALSE)),
    sql("GREATEST(`x`, `y`, `z`)")
  )
})
