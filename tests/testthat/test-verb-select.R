...
test_that("two selects equivalent to one", {
  mf <- local_memdb_frame(a = 1, b = 1, c = 1, d = 2)
...
test_that("correctly inlines across all verbs", {
  ...
  expect_selects(lf |> right_join(lf2, by = "x") |> select(x), 1)
...
test_that("select preserves grouping vars", {
  mf <- local_memdb_frame(a = 1, b = 2) |> group_by(b)
...
test_that("relocate works", {
  mf <- local_memdb_frame(a = 1, b = 2, c = 1) |> group_by(b)
...
test_that("select() after left_join() is inlined", {
  ...
  lq <- out$lazy_query
  expect_equal(lq$select$name, c("b", "x", "a"))
  expect_equal(lq$select$expr, unname(exprs(.table2$b, .table1$x, .table1$a)))
...
test_that("select() after join handles previous select", {
  ...
  lq2 <- lf2$lazy_query
  expect_equal(lq2$select$name, c("x2", "y3", "z"))
  expect_equal(lq2$select$expr, unname(exprs(.table1$x, .table1$y, .table1$z)))
...
test_that("select picks variables", {
  ...
  expect_equal(out$select, sql("\"x1\"", "\"x2\""))
})
...
test_that("select renames variables", {
  ...
  expect_equal(out$select, sql("\"x1\" AS \"y\"", "\"x2\" AS \"z\""))
})
...
test_that("select can refer to variables in local env", {
  ...
  expect_equal(out$select, sql("\"x\"", "\"y\""))
})
...
test_that("rename preserves existing vars", {
  ...
  expect_equal(out$select, sql("\"x\"", "\"y\" AS \"z\""))
})
