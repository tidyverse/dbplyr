...
test_that("as.numeric field coercion translate to MSSQL NUMERIC ", {
  expect_equivalent(translate_sql(as.numeric(field_name), con = mssql_connection),
                    sql("CAST(\"field_name\" AS NUMERIC)"))
})
test_that("as.double field coercion translate to MSSQL NUMERIC ", {
  expect_equivalent(translate_sql(as.double(field_name), con = mssql_connection),
                    sql("CAST(\"field_name\" AS NUMERIC)"))
})
test_that("as.integer field coercion translate to MSSQL INT ", {
  expect_equivalent(translate_sql(as.integer(field_name), con = mssql_connection),
                    sql("CAST(\"field_name\" AS INT)"))
})
test_that("as.logical field coercion translate to MSSQL BOOLEAN ", {
  expect_equivalent(translate_sql(as.logical(field_name), con = mssql_connection),
                    sql("CAST(\"field_name\" AS BOOLEAN)"))
})
test_that("as.character field coercion translate to MSSQL VARCHAR(MAX) ", {
  expect_equivalent(translate_sql(as.character(field_name), con = mssql_connection),
                    sql("CAST(\"field_name\" AS VARCHAR(MAX))"))
})
test_that("as.Date field coercion translate to MSSQL DATE ", {
  expect_equivalent(translate_sql(as.Date(field_name), con = mssql_connection),
...
test_that("| operator translate to MSSQL OR ", {
  expect_equivalent(translate_sql(field_name > 0 | field_name < 10, con = mssql_connection),
...
test_that("paste function translate to MSSQL CONCAT ", {
  expect_equivalent(translate_sql(paste0(field1, field2), con = mssql_connection),
