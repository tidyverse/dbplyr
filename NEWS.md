...
* `last_sql()` retrieves the most recent SQL query generated by dbplyr, which is useful for debugging (#1471).
* `memdb_frame()` with a data frame is now deprecated; it was never explicitly support but worked by coincidence. Use `copy_to(memdb(), df)` instead.
* `mutate()` gains `.order` and `.frame` arguments for specifying window function ordering and frame bounds within a single mutate call, similar to how `.by` works for grouping (#1542).
...
* Backend-specific improvements:
  * MS Access: correctly generates SQL for multiple joins by adding required parentheses (#1576).
  * MySQL: gains slightly better translation for `as.integer()` and `as.integer64()` (#1647).
  * Oracle: temporary tables use private temporary tables (Oracle 18c+) instead of global temporary tables. This ensures data persists correctly and table names are automatically prefixed with `ORA$PTT_` (#750).
  * PostgreSQL: improved translation for `seconds()`, `minutes()`, `hours()`, `days()`, `weeks()`, `months()`, and `years()`.
  * Postgres, Redshift, Snowflake, and Spark: new translations for `str_ilike()` (@edward-burn, #1628).
  * Redshift: `dbplyr_uncount()` works (@owenjonesuob, #1601); corrected error message for `quantile()` and `median()` in `mutate()` (@edward-burn, #1571); fixed syntax error in `date_build()` translation (#1512).
  * Snowflake: correctly translates `$` to `:` (@jsowder, #1608); fixed translations that were being reported as unknown (@edward-burn, #1570).
  * SQL Server: `between()` uses `CASE WHEN` instead of `IIF()` for compatibility with Azure Synapse (@rehbbea, #1773); `if_else()` uses `CASE WHEN` instead of `IIF` so `NULL` handling matches R's `NA` handling (#1569); `slice_sample()` returns different results each run (@thomashulst, #1503); `str_like()` and `str_ilike()` behave consistently (@edward-burn, #1669); on version 17.0 (2025) the stringr regex functions `str_detect()`, `str_starts()`, `str_ends()`, `str_replace()`, `str_replace_all()`, `str_remove()`, `str_remove_all()`, `str_extract()`, and `str_count()` are translated, and fixed-pattern versions of `str_detect()`, `str_starts()`, and `str_ends()` work on all versions (#1671); uses `DATEDIFF_BIG` instead of `DATEDIFF` so `tally()` and `count()` work regardless of data size (@edward-burn, #1666).
...
* `if_else()` and `ifelse()` give a clear error if `true`/`false` or `yes`/`no` are missing (#1798).
* `pivot_wider()` accepts anonymous functions (e.g. `\(x) max(x, na.rm = TRUE)`) in `values_fn`, not just purrr-style lambdas (#1816).
* `pull()` correctly handles a `NULL` quosure passed to `name` (#1808).
* `show_query()` gains a `use_colour` argument (#1590).
* `summarise()` reports grouping immediately, rather than when the SQL is generated (#1721).
