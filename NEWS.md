...
* Backend-specific improvements:
  ...
  * SQL Server: `between()` uses `CASE WHEN` instead of `IIF()` for compatibility with Azure Synapse (@rehbbea, #1773); `if_else()` uses `CASE WHEN` instead of `IIF` so `NULL` handling matches R's `NA` handling (#1569); `slice_sample()` returns different results each run (@thomashulst, #1503); `str_like()` and `str_ilike()` behave consistently (@edward-burn, #1669); on version 17.0 (2025) the stringr regex functions `str_detect()`, `str_starts()`, `str_ends()`, `str_replace()`, `str_replace_all()`, `str_remove()`, `str_remove_all()`, `str_extract()`, and `str_count()` are translated, and fixed-pattern versions of `str_detect()`, `str_starts()`, and `str_ends()` work on all versions (#1671); uses `DATEDIFF_BIG` instead of `DATEDIFF` so `tally()` and `count()` work regardless of data size (@edward-burn, #1666); `filter()` correctly casts logical columns to `BIT` when the filtered table is the right-hand side of a join (#1839).
  * Snowflake: correctly translates `$` to `:` (@jsowder, #1608); fixed translations that were being reported as unknown (@edward-burn, #1570); `pmin()` and `pmax()` with `na.rm = TRUE` use `LEAST_IGNORE_NULLS()` and `GREATEST_IGNORE_NULLS()` instead of a bespoke workaround (@mnelsonr, #1850).
